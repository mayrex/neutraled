...
export class GameRoom extends Room {
  onCreate(options) {
    ...
    this.onMessage('hit', (client, data) => {
        ...
        // Give some small evolution on hit
        if (shooter.evolution < 100 && shooter.role === 'human') {
            shooter.evolution = Math.min(100, shooter.evolution + (damage * 0.5)); // half damage as evolution points
        }
        ...
        if (role === 'monster' && weaponType === 1) {
             ...
             let interval = setInterval(() => {
                 ...
                 if (target.hp <= 0 && !target.isDead) {
                     ...
                     clearInterval(interval);
                     if (shooter.evolution < 100 && shooter.role === 'human') {
                         shooter.evolution = Math.min(100, shooter.evolution + 30);
                     }
                 }
             }, 2000);
        }
